...
class Loader(object):
    ...
    def load(path):
        ...
        try:
            ...
            sys.path.append(os.path.join(*items[:-1]))   #变量前加*表示可以传进来任意数目的变量
            ip_module = __import__(items[-1][:-3])   #__import__其实就像每次在程序开始用的 import方法
            return ip_module
...
def evaluate(settings):
    ...
    forecast_module = Loader.load(settings["pred_file"])  #加载predict.py文件
    ...
    day_acc = []
    for idx in range(0, preds.shape[0]):  #shape[0]是capacity，即风机编号总量
        acc = 1 - metrics.rmse(preds[idx, -day_len:, -1], gts[idx, -day_len:, -1]) / (settings["capacity"] * 1000)#这里我没有明白为何是计算一天的准确度而不是计算两天/是计算的两天
        if acc != acc:
            continue  #这里既然不是训练，为何会一直这样持续？
        day_acc.append(acc)
