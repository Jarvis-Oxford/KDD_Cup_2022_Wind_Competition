...
class Atten_En_Decoder(paddle.nn.Layer):
    def __init__(self, settings,use_teacher_forcing = True):
        ...
        #decoder
        self.lstm2 = nn.GRU(input_size= 1 + self.hidR,  #接收未来特征和encoder的输出
    ...
    def forward(self, x_enc, target):   #target是这个序列的真实预测值：[batch,outlen,1],1是预测的label维度
        """
        xf:future features  删去了xf，未来环境信息未知
        previous hidden:
        """
        ...
        for t in range(self.output_len):
            ...
            x_input = paddle.concat((decoder_output,context_vector), axis=-1)   #这里可能xf需要经过mlp变换一下输出的num_dims
            ...
            if self.use_teacher_forcing and teach_forcing is not None:
